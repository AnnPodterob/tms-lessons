...
# Вам необходимо создать класс Point (точка на координатной плоскости),
# у которой будет два поля: x, y.
# Добавьте метод distance_to_zero - который будет возвращать расстояние
# от точки до начала координат (0, 0). Например для точки Point(3, 4) это расстояние будет равно 5 (по теореме Пифагора)
...
class Point:
    ...
    def distance_to_point(self, p0):
        return ((p0.x - self.x)**2 + (p0.y - self.y)**2)**0.5
