...
def convert_bank_account_to_dict(bank_account: BankAccount) -> dict:
    return {
        'cart_holder': bank_account.card_holder,
...
class Bank:
    ...
    def external_transfer(self, from_account_number, to_external_number, money):
        from_account = self.bank_accounts[from_account_number]
        from_account.money -= money
        print(f'Банк перевёл {money}$ с вашего счёта {from_account} на внешний счёт {to_external_number}')
    ...
    def run(self):
        ...
        while True:
            ...
            if act == 0:
                ...
                money = float(input('Введиет количество денег, которые вы хотите положить на соответствующий счёт: '))
                ...
                from_account_number = input('Введите номер счёта-отправителя: ')
                to_account_number = input('Введите номер внешнего счёта: ')
                money = float(input('Введите количество денег: '))
